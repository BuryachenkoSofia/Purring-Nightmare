...
public class whiskas : MonoBehaviour
{
  ...
  void FixedUpdate()
  {
    ...
    {
      transform.position = Vector2.MoveTowards(transform.position, player.position, whiskasSpeed * Time.deltaTime);
    }
    ...
    {
      transform.position = Vector2.MoveTowards(transform.position, firstPosition, whiskasSpeed * Time.deltaTime);
    }
  }
}